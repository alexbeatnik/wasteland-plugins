...
| Plugin | What it adds |
|---|---|
| [`audio-player`](plugins/audio-player) | Plays music from a folder on your machine, with a transport bar in the chat window. Reads ID3 and Vorbis tags out of MP3, FLAC, Ogg and Opus, so it knows who performed a track even when the file name does not say. The model can start a song, build a playlist, skip and pause — and when several files could be what you meant, you get a list to click. A playlist gathered from more than one album comes up shuffled, so a band does not play alphabetically every time; an album keeps its own running order. |
